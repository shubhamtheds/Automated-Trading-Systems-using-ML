...
class MeanRevBacktester():
    ...
    def __init__(self, symbol, SMA, dev, start, end, interval, ptc):
        '''
        Parameters
        ----------
        symbol: str
...
#         self.data = raw
        ...
        raw = yf.download(self.symbol, self.start, self.end, interval=str(self.interval)).Close.to_frame()
        raw.rename(columns = {"Close":self.symbol}, inplace = True)
        raw = raw[self.symbol].to_frame().dropna()
